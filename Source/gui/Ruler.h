...
namespace gui
{
        ...
        public Comp
    {
		...
		void setGetIncFunc(std::function<float(float)>&& f) noexcept
		{
			getIncFunc = f;
		}

		void setValToStrFunc(std::function<String(float)>&& f) noexcept
		{
			valToStrFunc = f;
		}

		void setDrawFirstVal(bool e) noexcept
		{
			drawFirstVal = e;
		}
		...
		void setLength(float l) noexcept
		{
			if (length == l)
				return;
			length = l;
		...
		void paintStripes(Graphics& g, float top, float btm, int numStripes)
		{
