...
namespace moog
{
	static double S_STILSON_GAINTABLE[199] =
	{
		...
		public LadderFilterBase
	{
		...
		void setResonance(double r) noexcept override
		{
			...
			const auto ix = p * 99.;
			const auto ixfloor = floor(ix);
			const auto ixint = static_cast<int>(ixfloor);
			const auto ixfrac = ix - ixint;
		...
		void setCutoff(double c) noexcept override
		{
			...
			const auto x2 = fc * fc;
			const auto x3 = fc * fc * fc;

			// Frequency & amplitude correction (Cubic Fit)
			p = -.69346 * x3 - .59515 * x2 + 3.2937 * fc - 1.0072;
